...
### Added
- **Validation support** via `[NotifyDataErrorInfo]` attribute and `ObservableValidator` base class. Properties annotated with `[ObservableProperty]` and `[NotifyDataErrorInfo]` (or on a class with `[NotifyDataErrorInfo]`) automatically call `ValidateProperty(value, nameof(Property))` in the generated setter. `ObservableValidator` implements `INotifyDataErrorInfo`, providing `ValidateProperty()`, `ValidateAllProperties()`, `ClearErrors()`, and `ClearAllErrors()` methods backed by `System.ComponentModel.DataAnnotations`.
- New diagnostic **PSG5001** (Warning): `[NotifyDataErrorInfo]` is used but the containing type does not inherit from `ObservableValidator`; validation calls will not be emitted.
